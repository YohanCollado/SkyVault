...
export const covertFileSize = (sizeInBytes: number, digits?: number) => {
  if (sizeInBytes < 1024) {
    return sizeInBytes + "Bytes"; // < than 1 Kb, show in bytes  
  } else if (sizeInBytes < 1024 * 1024) {
    const sizeInKB = sizeInBytes / 1024;
    return sizeInKB.toFixed(digits || 1) + " KB"; // < than 1 MB, shown in KB
  } else if (sizeInBytes < 1024 * 1024 * 1024) {
    const sizeInMB = sizeInBytes / (1024 * 1024);
    return sizeInMB.toFixed(digits || 1) + " MB"; // < than 1 GB, shown in MB
  } else {
    const sizeInGB = sizeInBytes / (1024 * 1024 * 1024);
    return sizeInGB.toFixed(digits || 1) + " GB"; // if more than 1 gb shown in GB
  }
};
...
export const getFileType = (fileName: string) => {
  ...
  const videoExtension = ["mp4", "avi", "mov", "mkv", "webm"];
  const audioExtension = ["mp3", "wav", "ogg", "flac"];
...
export const formatDateTime = (isoString: string | null | undefined) => {
  ...
  //convert hours to 12 format (standard)
  hours = hours %12 || 12;
